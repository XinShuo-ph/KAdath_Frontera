...
TEST(KAdathTestIndex,CheckArrayIndex)
{
    ...
    Dim_array_iterator array_index{dimensions};
    {
        bool inc_ok{true};
        bool inc_i{true};
        bool inc_ai{true};
    ...
    }
    {
        bool inc_i{true};
        bool inc_ai{true};
        for(int var=0;var<ndim;var++) for(int step=2;step<d[var];step++) {
            index.set_start();
            array_index.set_start();
