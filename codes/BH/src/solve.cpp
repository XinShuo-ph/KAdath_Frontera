...
template<typename space_t, typename syst_t, typename config_t>
void print_diagnostics_pre(space_t const & space, syst_t const & syst, int const ite, double const conv, config_t& bconfig);
...
int main(int argc, char** argv) {
  ...
  Scalar coord_scalars {Scalar(space)};
  coord_scalars.annule_hard();
  coord_scalars.std_base();
  ...
  if(stage_enabled[PRE]) {
    ...
    Scalar level(space);
    level = coord_scalars * coord_scalars - bconfig(RMID) * bconfig(RMID);
    level.std_base();
