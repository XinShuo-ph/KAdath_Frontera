...
int main(int argc, char **argv) {
  if (argc < 2) {
    std::cerr << "Input file name missing..." << endl;
    std::_Exit(EXIT_FAILURE);
  }

  if (argc < 3) {
    std::cerr << "Output file name missing..." << endl;
    std::_Exit(EXIT_FAILURE);
  }

  if (argc < 4) {
    std::cerr << "Missing new resolution" << std::endl;
    std::_Exit(EXIT_FAILURE);
  }
  ...
  // this allows converting old ID
  if(std::isnan(bconfig.set(NINSHELLS, BCO1))) {
    bconfig.set(NINSHELLS, BCO1) = bconfig(NSHELLS, BCO1);
  ...
  // print bounds to stdout
  std::cout << "Bounds:" << std::endl;
	bco_utils::print_bounds("NS", NS_bounds);
	bco_utils::print_bounds("BH", BH_bounds);
  std::cout << std::endl;

  Space_bhns space (type_coloc, bconfig(DIST), NS_bounds, BH_bounds, out_bounds, bconfig(BIN_RES), bconfig(NINSHELLS));
