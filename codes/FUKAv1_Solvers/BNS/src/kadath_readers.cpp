...
class bns_reader_t : public Kadath::python_reader_t<space_t, bns_vars_t> {
  ...
  void compute_defs() {
    ...
    vars["S1"] = space.get_domain(space.ADAPTED1+1)->integ(syst.give_val_def("intS1")()(space.ADAPTED1+1), OUTER_BC);
    vars["S2"] = space.get_domain(space.ADAPTED2+1)->integ(syst.give_val_def("intS2")()(space.ADAPTED2+1), OUTER_BC);
