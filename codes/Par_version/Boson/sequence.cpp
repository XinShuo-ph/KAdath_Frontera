...
int main(int argc, char **argv) {
    ...
    for (int kant = 0; kant < number; kant++) {
        ...
        syst.add_def(ndom - 1,
                     "eqnu = lap(nu) - B^2*rsint^2/2/N^2 * scal(multr(grad(Np)) , multr(grad(Np))) + scal(grad(nu), grad(nu+log(B))) - qpi*A^2*(E+S)");


        for (int d = 0; d < ndom - 1; d++)
            syst.add_def(d,"eqA = lap2(incA) - 2*qpi*A^2*Spp- 3 * B^2 * rsint^2 /4 / N^2 * scal(grad(Np) , grad(Np)) + scal(grad(nu),grad(nu))");
        syst.add_def(ndom - 1,
                     "eqA =lap2(incA) - 2*qpi*A^2*Spp- 3 * B^2 * rsint^2 /4 / N^2 * scal(multr(grad(Np)) , multr(grad(Np))) + scal(grad(nu),grad(nu)) ");
