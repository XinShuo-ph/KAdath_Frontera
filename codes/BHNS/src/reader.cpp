...
void reader_output(config_t bconfig, const int output) {
  ...
  double A = space.get_domain(space.ADAPTEDNS+1)->integ(syst.give_val_def("intMsq")()(space.ADAPTEDNS+1), INNER_BC);
  double areal_r =  sqrt(A);
  ...
  std::cout   << FORMAT1<< "RES = "             << bconfig(BIN_RES)                                              << std::endl
              << FORMAT1<< "NS SHELLS = "       << nshells1                                                      << std::endl
              << FORMAT1<< "NS R_IN = "         << rin_ns                                                        << std::endl;
  print_shells(space.NS+1, space.ADAPTEDNS);
  std::cout   << FORMAT1<< "NS R_SUR = "        << "[" << rmin << "," << rmax << "] ("
                                                << "[" << rmin * M2km << "," << rmax * M2km << "] km)"           << std::endl
              << FORMAT1<< "NS AREAL = "        << areal_r                                                       << std::endl 
              << FORMAT1<< "NS R_OUT = "        << rout_ns                                                       << std::endl
              << FORMAT1<< "Center_COM = " << "(" << NS_x_com << ", 0, 0)\n"
              << FORMAT << "NS Mb = "           << MB1 << " (";
  ...
  std::cout                                     << ")"                                                           << std::endl
              ...
              << FORMAT1<< "BH SHELLS = "       << nshells2                                                      << std::endl
              << FORMAT1<< "BH R_IN = "         << rin_bh                                                        << std::endl
              << FORMAT1<< "BH R_SUR = "        << r_bh                                                     
                        << " ["                 << r_bh * M2km << "km]"                                          << std::endl
              << FORMAT1 <<" LAPSE = ["         << lapsemin << ", " << lapsemax  <<"]\n"
              << FORMAT1 <<" PSI = ["           << confmin  << ", " << confmax   <<"]\n";

  print_shells(space.BH+2, space.OUTER-1);
  std::cout   << FORMAT1<< "BH R_OUT = "        << rout_bh                                                       << std::endl
              << FORMAT1<< "Center_COM = " << "(" << BH_x_com << ", 0, 0)\n"
              << FORMAT << "BH QLS = "          << ql_spinbh << " (chi = " << ql_spinbh / (mch * mch) << ")"     << std::endl
              << FORMAT << "BH Omega = "        << bconfig(OMEGA,BCO2)                                           << std::endl
              << FORMAT << "BH Mirr = "         << mirr << " (Mch = " << mch << ")"                              << "\n\n";
  ...
  std::cout   << FORMAT1<< "Q = "               << Madm1 / mch                                                   << std::endl
              << FORMAT << "GOmega = "          << bconfig(GOMEGA)                                               << std::endl
              << FORMAT << "Komar mass = "      << komar                                                         << std::endl
              << FORMAT << "Adm   mass = "      << adm_inf << ", Diff: " << e_diff                               << std::endl
              ...
              << FORMAT << "COMx = "            << bconfig(COM)                                                  << std::endl
              << FORMAT << "COMy = "            << bconfig(COMY)                                                 << std::endl
              << FORMAT << "aCOMx = "           << COMx                                                          << std::endl
              << FORMAT << "aCOMy = "           << COMy                                                          << std::endl
              << FORMAT << "aCOMz = "           << COMz                                                          << std::endl
