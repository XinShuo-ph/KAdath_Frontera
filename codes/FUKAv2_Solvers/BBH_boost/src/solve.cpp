...
int main(int argc, char** argv) {
  ...
  if(InitSolver::example_setup) {
    std::cout << "Generating example setup." << std::endl;
    if(rank == 0) {
    ...
    std::cout << "Running solver from existing configuration." << std::endl;
    bconfig.open_config();
    bconfig.control(CONTROLS::SEQUENCES) = InitSolver::setup_first;
    ...
    bconfig.set_stage(STAGES::LINBOOST) = true;// manually set for now
    ...
    if(!(seq.is_set() || seq_bin.is_set()) && !bconfig.control(CONTROLS::SEQUENCES)) {
      std::cout << "Running single calculation." << std::endl;
      ...
      if(seq.is_set()) {
        std::cout << "Running sequence calculation." << std::endl;
