...
Term_eq Ope_atanh::action() const {
	...
	switch (target.type_data) {
		...
		cerr << "Ope_atanh only defined with respect for a scalar" << endl ;
		abort() ;
	    }
	...
	for (int i=0 ; i<target.val_t->get_n_comp() ; i++) {
		...
		resval.set(ind).set_domain(dom) = atanh(value) ;
	}
	...
	if (target.der_t!=0x0) {
		...
		for (int i=0 ; i<target.der_t->get_n_comp() ; i++) {
			...
			Val_domain value ((*target.val_t)(ind)(dom)) ;
			resder.set(ind).set_domain(dom) = valder/(1 - value*value) ;
			}
	...
	case TERM_D : {
	      if (target.der_d==0x0) {
		  Term_eq res (dom, atanh(*target.val_d)) ;
		  return res ;
	      }
	      else {
		  Term_eq res (dom, atanh(*target.val_d), *target.der_d/(1 - *target.val_d * *target.val_d)) ;
		  return res ;
	      }
	  }
	  break ;
    ...
    cerr << "Warning should not be here in Ope_atanh::action..." << endl ;
    abort() ;
}
}
