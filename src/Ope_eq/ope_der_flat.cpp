...
namespace Kadath {
Ope_der_flat::Ope_der_flat (const System_of_eqs* zesys, int tt, char ind, Ope_eq* target) : Ope_eq(zesys, target->get_dom(), 1), 
...
Term_eq Ope_der_flat::action() const {
	...
	if (target.type_data != TERM_T) {
		cerr << "Ope_der_flat only defined with respect for a tensor" << endl ;
		abort() ;
	}
