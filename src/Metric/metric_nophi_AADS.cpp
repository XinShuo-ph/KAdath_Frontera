...
Metric_nophi_AADS::Metric_nophi_AADS (Metric_tensor& met, const Scalar& ome) : 
		Metric(met.get_space()), p_met(&met), basis(met.get_basis()), fmet(met.get_space(), basis), conformal(ome), der_conf (conformal.der_r()) {
	type_tensor = met.get_type() ;
	for (int d=0 ; d<met.get_space().get_nbr_domains() ; d++)
	if (basis.get_basis(d) != SPHERICAL_BASIS)  {
		cerr << "Metric_nophi_AADS only defined wrt spherical tensorial coordinates" << endl ;
		abort() ;
	}
...
Metric_nophi_AADS::Metric_nophi_AADS (const Metric_nophi_AADS& so) : 
		Metric (so), p_met(so.p_met), basis(so.basis), fmet(so.fmet), conformal(so.conformal), der_conf(conformal.der_r()), place_syst(so.place_syst) {
}
...
void Metric_nophi_AADS::compute_christo (int dd) const {
	...
	do {
		...
		cmpval += (*p_met_con[dd]->val_t)(pos(2)+1,1)(dd)*(*p_met_cov[dd]->val_t)(pos(0)+1,pos(1)+1)(dd)*der_conf(dd) ;
		...
		// Some corrections due to the conformal factor :
		cmpder += ((*p_met_con[dd]->der_t)(pos(2)+1,1)(dd)*(*p_met_cov[dd]->val_t)(pos(0)+1,pos(1)+1)(dd)
				+(*p_met_con[dd]->val_t)(pos(2)+1,1)(dd)*(*p_met_cov[dd]->der_t)(pos(0)+1,pos(1)+1)(dd)) *der_conf(dd) ;
