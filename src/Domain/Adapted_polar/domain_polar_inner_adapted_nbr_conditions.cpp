	...
	do {
		...
		switch (baset) {
					...
					case SIN_ODD:
						...
						break ;
					default:
						cerr << "Unknow theta basis in Domain_polar_shell_inner_adapted::nbr_conditions_val_domain" << endl ;
						abort() ;
		}
...
Array<int> Domain_polar_shell_inner_adapted::nbr_conditions (const Tensor& tt, int dom, int order, int n_cmp, Array<int>**) const {
	...
	switch (val) {
		case 0 :
			...
			break ;
		default :
			cerr << "Valence " << val << " not implemented in Domain_polar_shell_inner_adapted::nbr_conditions" << endl ;
			break ;
	}
	return res ;
}
