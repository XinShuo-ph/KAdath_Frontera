...
Term_eq Domain_polar_shell_inner_adapted::der_normal_term_eq (const Term_eq& so, int bound) const {
  ...
  switch (bound) {
    ...
    default : 
	cerr << "Unknown boundary in Domain_polar_shell_inner_adapted::der_normal" << endl ;
	abort() ;
  }
}
...
Term_eq Domain_polar_shell_inner_adapted::lap_term_eq (const Term_eq& so, int mm) const {
   ...
   if (so.get_val_t().get_valence() != 0) {
      cerr << "Domain_polar_shell_inner_adapted::lap_term_eq only defined for scalars" << endl ;
      abort() ;
  }
...
Term_eq Domain_polar_shell_inner_adapted::lap2_term_eq (const Term_eq& so, int mm) const {
    if (mm!=0) {
      cerr << "Domain_polar_shell_inner_adapted::lap2_term_eq not defined for m != 0 (for now)" << endl ;
      abort() ;
  }
   
   if (so.get_val_t().get_valence() != 0) {
      cerr << "Domain_polar_shell_inner_adapted::lap2_term_eq only defined for scalars" << endl ;
      abort() ;
  }
...
const Term_eq* Domain_polar_shell_inner_adapted::give_normal (int bound, int tipe) const {
  ...
  switch (tipe) {
    ...
    default: 
      cerr << "Unknown type of tensorial basis in Domain_polar_shell_inner_adapted::give_normal" << endl ;
      abort() ;
  }
}
...
Term_eq Domain_polar_shell_inner_adapted::integ_term_eq (const Term_eq&, int) const {
  cerr << "Domain_polar_shell_inner_adapted::integ_term_eq not implemented yet" << endl ;
  abort() ;
}
}
