...
namespace Kadath {
void Domain_shell_inner_adapted::export_tau_val_domain_boundary (const Val_domain& so, int mlim, int bound, Array<double>& sec, int& pos_sec, int ncond) const {
			...
			for (int j=0 ; j<nbr_coefs(1) ; j++) {
				...
				switch (baset) {
					...
					case SIN_ODD:
						...
						break ;
					default:
						cerr << "Unknow theta basis in Domain_shell_inner_adapted::export_tau_val_domain_boundary" << endl ;
						abort() ;
					}
				}
		}
...
void Domain_shell_inner_adapted::export_tau_boundary (const Tensor& tt, int dom, int bound, Array<double>& res, int& pos_res, const Array<int>& ncond,
										int n_cmp, Array<int>** p_cmp) const {

	// Check boundary
	if ((bound!=INNER_BC) && (bound!=OUTER_BC)) {
		cerr << "Unknown boundary in Domain_shell_inner_adapted::export_tau_boundary" << endl ;
		abort() ;
	}
	...
	switch (val) {
		...
		case 1 : {
			...
			if (!found) {
				cerr << "Unknown type of vector Domain_shell_inner_adapted::export_tau_boundary" << endl ;
				abort() ;
			}
		}
			break ;
		case 2 : {
			...
			if (!found) {
				cerr << "Unknown type of 2-tensor Domain_shell_inner_adapted::export_tau_boundary" << endl ;
				abort() ;
			}
		}
			break ;
		default :
			cerr << "Valence " << val << " not implemented in Domain_shell_inner_adapted::export_tau_boundary" << endl ;
			break ;
	}
}
}
