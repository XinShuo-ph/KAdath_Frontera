	...
	do {
		...
		switch (baset) {
					...
					case SIN_ODD:
						...
						break ;
					default:
						cerr << "Unknow theta basis in Domain_shell_inner_adapted::nbr_unknowns_val_domain" << endl ;
						abort() ;
		}
		if (indic)
			res ++ ;
...
int Domain_shell_inner_adapted::nbr_unknowns (const Tensor& tt, int dom) const {
	...
	switch (val) {
		...
		case 1 : {
			...
			if (!found) {
				cerr << "Unknown type of vector Domain_shell_inner_adapted::nbr_unknowns" << endl ;
				abort() ;
			}
		}
		    break ;
		case 2 : {
			...
			if (!found) {
				cerr << "Unknown type of 2-tensor Domain_shell_inner_adapted::nbr_unknowns" << endl ;
				abort() ;
			}
		}
		break ;
