	...
	do {
		...
		switch (baset) {
					...
					case SIN_ODD:
						...
						break ;
					default:
						cerr << "Unknow theta basis in Domain_shell_outer_adapted::nbr_conditions_val_domain" << endl ;
						abort() ;
		}
...
Array<int> Domain_shell_outer_adapted::nbr_conditions (const Tensor& tt, int dom, int order, int n_cmp, Array<int>** p_cmp) const {
	...
	switch (val) {
		...
		case 1 : {
			...
			if (!found) {
				cerr << "Unknown type of vector Domain_shell_outer_adapted::nbr_conditions" << endl ;
				abort() ;
			}
		}
		      break ;
		case 2 : {
			...
			if (!found) {
				cerr << "Unknown type of 2-tensor Domain_shell_outer_adapted::nbr_conditions" << endl ;
				abort() ;
			}
		}
			break ;
