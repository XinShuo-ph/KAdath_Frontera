			...
			for (int j=0 ; j<nbr_coefs(1) ; j++) {
				...
				switch (baset) {
					...
					case SIN_ODD:
						...
						break ;
					default:
						cerr << "Unknow theta basis in Domain_shell_inner_adapted::affecte_one_coef_val_domain" << endl ;
						abort() ;
					}
...
void Domain_shell_inner_adapted::affecte_tau_one_coef (Tensor& tt, int dom, int cc, int& pos_cf) const {
	...
	switch (val) {
		...
		case 1 : {
			...
			if (!found) {
				cerr << "Unknown type of vector Domain_shell_inner_adapted::affecte_tau_one_coef" << endl ;
				abort() ;
			}
		}
			break ;
		case 2 : {
			...
			if (!found) {
				cerr << "Unknown type of 2-tensor Domain_shell_inner_adapted::affecte_tau_one_coef" << endl ;
				abort() ;
			}
		}
			break ;
