...
Domain_spheric_periodic_compact::Domain_spheric_periodic_compact (int num, int ttype, int typet, double rint, double oome, const Dim_array& nbr) :  
...
// Constructor by copy
Domain_spheric_periodic_compact::Domain_spheric_periodic_compact (const Domain_spheric_periodic_compact& so) : Domain(so), alpha(so.alpha), ome(so.ome),
					  type_time(so.type_time), maxt (so.maxt) {
}

Domain_spheric_periodic_compact::Domain_spheric_periodic_compact (int num, FILE* fd) : Domain(num, fd) {
...
void Domain_spheric_periodic_compact::save (FILE* fd) const {
	nbr_points.save(fd) ;
	nbr_coefs.save(fd) ;
...
void Domain_spheric_periodic_compact::do_coloc () {
	...
	switch (type_base) {
		...
		case LEG_TYPE:
			...
			break ;
		default :
			cerr << "Unknown type of basis in Domain_spheric_periodic_compact::do_coloc" << endl ;
			abort() ;
	}
}
...
void Domain_spheric_periodic_compact::do_der_abs_from_der_var(const Val_domain *const *const der_var, Val_domain **const der_abs) const {
	// d/dr
