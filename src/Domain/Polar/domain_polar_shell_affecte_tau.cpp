...
#include "tensor.hpp"
namespace Kadath {
void Domain_polar_shell::affecte_tau_val_domain (Val_domain& so, int mquant, const Array<double>& values, int& conte) const {
	...
	for (int j=0 ; j<nbr_coefs(1) ; j++) {
		...
		switch (baset) {
			...
			case SIN_ODD:
				...
				break ;
			default:
			cerr << "Unknow theta basis in Domain_polar_shell::affecte_tau_val_domain" << endl ;
				abort() ;
		}
