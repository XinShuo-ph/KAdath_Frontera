...
namespace Kadath {
void Domain_polar_compact::export_tau_val_domain_boundary (const Val_domain& so, int mquant, int bound, Array<double>& sec, int& pos_sec, int ncond) const {
	...
	for (int j=0 ; j<nbr_coefs(1) ; j++) {
		...
		switch (baset) {
			...
			case SIN_ODD:
				...
				break ;
			default:
				cerr << "Unknow theta basis in Domain_polar_compact::export_tau_val_domain_boundary" << endl ;
				abort() ;
		  }
		}
	}
...
void Domain_polar_compact::export_tau_boundary (const Tensor& tt, int dom, int bound, Array<double>& res, int& pos_res, const Array<int>& ncond,
										int, Array<int>**) const {

	// Check boundary
	if ((bound!=INNER_BC) && (bound!=OUTER_BC)) {
		cerr << "Unknown boundary in Domain_polar_compact::export_tau_boundary" << endl ;
		abort() ;
	}
