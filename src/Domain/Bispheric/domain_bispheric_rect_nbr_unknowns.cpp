...
int Domain_bispheric_rect::nbr_unknowns_val_domain (const Val_domain& so) const {
	...
	do {
		...
		case SIN :
			...
			break ;
		default :
			cerr << "Unknown phi basis in Domain_bispheric_rect::nbr_unknowns_val_domain" << endl ;
			abort() ;
		}
		if (indic)
			res ++ ;
...
int Domain_bispheric_rect::nbr_unknowns (const Tensor& tt, int dom) const {
	...
	switch (val) {
		...
		case 1 : {
			...
			if (!found) {
				cerr << "Unknown type of vector Domain_bispheric_rect::nbr_unknowns" << endl ;
				abort() ;
			}
		}
		    break ;
		case 2 : {
			...
			if (!found) {
				cerr << "Unknown type of 2-tensor Domain_bispheric_rect::nbr_unknowns" << endl ;
				abort() ;
			}
		}
		break ;
