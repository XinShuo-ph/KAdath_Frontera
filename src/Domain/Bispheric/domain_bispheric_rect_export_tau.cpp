...
namespace Kadath {
void Domain_bispheric_rect::export_tau_val_domain (const Val_domain& so, int order, Array<double>& sec, int& pos_sec, int ncond) const {
							...
							if ((k==0) || (k%2==1)) {
								...
								switch (type_base) {
									...
									case LEG_TYPE :
										...
										break ;
									default :
						cerr << "Unknown type of basis in Domain_bispheric_rect::export_tau_val_domain" << endl ;
										abort() ;
								}
							pos_galerkin = pos_cf ;
...
void Domain_bispheric_rect::export_tau (const Tensor& tt, int dom, int order, Array<double>& res, int& pos_res, const Array<int>& ncond,
	...
	switch (val) {
		...
		case 1 : {
			...
			if (!found) {
				cerr << "Unknown type of vector Domain_bispheric_rect::export_tau" << endl ;
				abort() ;
			}
		}
			break ;
		case 2 : {
			...
			if (!found) {
				cerr << "Unknown type of 2-tensor Domain_bispheric_rect::export_tau" << endl ;
				abort() ;
			}
		}
			break ;
