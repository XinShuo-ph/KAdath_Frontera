	...
	*so.cf=0. ;
	...
	do {	
		...
		case SIN :
			...
			break ;
		default :
			cerr << "Unknown phi basis in Domain_bispheric_rect::affecte_tau_val_domain" << endl ;
			abort() ;
		}
...
void Domain_bispheric_rect::affecte_tau_one_coef (Tensor& tt, int dom, int cc, int& pos_cf) const {
	...
	switch (val) {
		...
		case 1 : {
			...
			if (!found) {
				cerr << "Unknown type of vector Domain_bispheric_rect::affecte_tau_one_coef" << endl ;
				abort() ;
			}
		}
			break ;
		case 2 : {
			...
			if (!found) {
				cerr << "Unknown type of 2-tensor Domain_bispheric_rect::affecte_tau_one_coef" << endl ;
				abort() ;
			}
		}
			break ;
