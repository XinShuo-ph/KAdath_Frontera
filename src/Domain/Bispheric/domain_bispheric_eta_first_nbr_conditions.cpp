	...
	for (int k=0 ; k<nbr_coefs(2) ; k++)
		...
		for (int i=0 ; i<nbr_coefs(0) ; i++) {
			...
			switch (basep) {
				...
				break ;
			default :
				cerr << "Unknwon phi basis in Domain_bispheric_eta_first:nbr_conditions_val_domain" << endl ;
				abort() ;
			}
...
Array<int> Domain_bispheric_eta_first::nbr_conditions (const Tensor& tt, int dom, int order, int n_cmp, Array<int>** p_cmp) const {
	...
	switch (val) {
		...
		case 1 : {
			...
			if (!found) {
				cerr << "Unknown type of vector Domain_bispheric_eta_first::nbr_conditions" << endl ;
				abort() ;
			}
		}
		      break ;
		case 2 : {
			...
			if (!found) {
				cerr << "Unknown type of 2-tensor Domain_bispheric_eta_first::nbr_conditions" << endl ;
				abort() ;
			}
		}
			break ;
