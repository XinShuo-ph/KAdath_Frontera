...
Domain_critic_inner::Domain_critic_inner (int num, int ttype, const Dim_array& nbr, double xx) : Domain(num, ttype, nbr), p_X(nullptr), p_T(nullptr), xlim(xx) {
     do_coloc() ;
}
...
void Domain_critic_inner::del_deriv() {
...
const Val_domain & Domain_critic_inner::get_X() const {
  if (p_X==nullptr)
		do_X() ;
	return *p_X ;
}

const Val_domain & Domain_critic_inner::get_T() const {
  if (p_T==nullptr)
		do_T() ;
	return *p_T ;
}
