...
Array<int> Domain_critic_inner::nbr_conditions_boundary (const Tensor& tt, int dom, int bound, int n_cmp, Array<int>**) const {

	// Check boundary
	if ((bound!=INNER_BC) && (bound!=OUTER_BC)) {
		cerr << "Unknown boundary in Domain_critic_inner::nbr_conditions_boundary" << endl ;
		abort() ;
	}
