	...
	for (int j=0 ; j<nbr_coefs(1) ; j++) {
		...
		switch (basetime) {
					...
					case SIN:
						...
						break ;
					default:
						cerr << "Unknow time basis in Domain_polar_polar_nucleus::nbr_conditions_val_domain_boundary" << endl ;
						abort() ;
		}
		...
		switch (baset) {
					...
					case SIN_ODD:
						...
						break ;
					default:
						cerr << "Unknow theta basis in Domain_polar_periodic_nucleus::nbr_conditions_val_boundary" << endl ;
						abort() ;
		}
...
Array<int> Domain_polar_periodic_nucleus::nbr_conditions_boundary (const Tensor& tt, int dom, int bound, int n_cmp, Array<int>** p_cmp) const {
	...
	switch (val) {
		...
		case 2 :
			...
			break ;
		default :
			cerr << "Valence " << val << " not implemented in Domain_polar_periodic_nucleus::nbr_conditions_boundary" << endl ;
			break ;
	}
	return res ;
}}
