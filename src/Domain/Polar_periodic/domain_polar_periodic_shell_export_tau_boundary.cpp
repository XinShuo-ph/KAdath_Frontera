...
namespace Kadath {
void Domain_polar_periodic_shell::export_tau_val_domain_boundary (const Val_domain& so, int bound, Array<double>& sec, int& pos_sec, int ncond) const {
	...
	switch (basetime) {
		...
		case SIN :
			...
			break ;
		default :
			cerr << "Unknown time basis in Domain_polar_periodic_shell_export_tau_val_domain_boundary" << endl ;
			abort() ;
	}
...
void Domain_polar_periodic_shell::export_tau_boundary (const Tensor& tt, int dom, int bound, Array<double>& res, int& pos_res, const Array<int>& ncond,
	...
	switch (val) {
		...
		case 2 :
			...
			break ;
		default :
			cerr << "Valence " << val << " not implemented in Domain_polar_periodic_shell::export_tau_boundary" << endl ;
			abort() ;
			break ;
	}
}}
