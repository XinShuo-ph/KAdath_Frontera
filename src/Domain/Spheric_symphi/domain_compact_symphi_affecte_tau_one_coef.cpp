	...
	int kmin, kmax ;
		...
		switch (basep) {
					...
					case SIN_ODD:
						...
						break ;
					default:
						cerr << "Unknow phi basis in Domain_compact_symphi::affecte_tau_one_coef_val_domain" << endl ;
						abort() ;
		}
...
void Domain_compact_symphi::affecte_tau_one_coef (Tensor& tt, int dom, int cc, int& pos_cf) const {
	...
	switch (val) {
		...
		case 1 : {
			...
			if (!found) {
				cerr << "Unknown type of vector Domain_compact_symphi::affecte_tau_one_coef" << endl ;
				abort() ;
			}
		}
			break ;
		case 2 : {
			...
			if (!found) {
				cerr << "Unknown type of 2-tensor Domain_compact_symphi::affecte_tau_one_coef" << endl ;
				abort() ;
			}
		}
			break ;
