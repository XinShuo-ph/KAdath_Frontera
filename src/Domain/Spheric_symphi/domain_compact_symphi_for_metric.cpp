...
namespace Kadath {
Term_eq Domain_compact_symphi::derive_flat_spher (int type_der, char ind_der, const Term_eq& so, const Metric* manipulator) const {
	...
	do {
		...
		switch (pos_auxi_bis(0)) {
		    ...
		    case 2 :
		      ...
		      break ;
		    default :
		      cerr << "Bad indice in Domain_compact_symphi::derive_flat_spher" << endl  ;
		      abort() ;
		}
	}
...
Term_eq Domain_compact_symphi::derive_flat_cart (int type_der, char ind_der, const Term_eq& so, const Metric* manipulator) const {
