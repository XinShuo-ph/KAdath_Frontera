	...
	int kmin, kmax ;
		...
		switch (basep) {
					...
					case SIN_ODD:
						...
						break ;
					default:
						cerr << "Unknow phi basis in Domain_shell_symphi::affecte_tau_val_domain" << endl ;
						abort() ;
		}
...
void Domain_shell_symphi::affecte_tau (Tensor& tt, int dom, const Array<double>& cf, int& pos_cf) const {
	...
	switch (val) {
		...
		case 1 : {
			...
			if (!found) {
				cerr << "Unknown type of vector Domain_shell_symphi::affecte_tau" << endl ;
				abort() ;
			}
		}
			break ;
		case 2 : {
			...
			if (!found) {
				cerr << "Unknown type of 2-tensor Domain_shell_symphi::affecte_tau" << endl ;
				abort() ;
			}
		}
			break ;
