...
void Domain_shell::affecte_tau_val_domain_mquant (Val_domain& so, int mquant, const Array<double>& values, int& conte) const {
	...
	for (int j=0 ; j<nbr_coefs(1) ; j++) {
		...
		switch (baset) {
			...
			case SIN_ODD:
				...
				break ;
			default:
			cerr << "Unknow theta basis in Domain_shell::affecte_tau_val_domain_mquant" << endl ;
				abort() ;
		}
			...
			for (int j=0 ; j<nbr_coefs(1) ; j++) {
				...
				switch (baset) {
					...
					case SIN_ODD:
						...
						break ;
					default:
			cerr << "Unknow theta basis in Domain_shell::affecte_tau_val_domain" << endl ;
						abort() ;
					}
...
void Domain_shell::affecte_tau (Tensor& tt, int dom, const Array<double>& cf, int& pos_cf) const {
	...
	switch (val) {
		...
		case 1 : {
			...
			if (!found) {
				cerr << "Unknown type of vector Domain_shell::affecte_tau" << endl ;
				abort() ;
			}
		}
			break ;
		case 2 : {
			...
			if (!found) {
				cerr << "Unknown type of 2-tensor Domain_shell::affecte_tau" << endl ;
				abort() ;
			}
		}
			break ;
