...
void Domain_compact::export_tau_val_domain_mquant (const Val_domain& so, int mquant, int order, Array<double>& sec, int& pos_sec, int ncond) const {
	if (so.check_if_zero()) 
		pos_sec += ncond ;
	else {
		...
		for (int i=0 ; i<nbr_coefs(0)-order ; i++) {
			...
			switch (baset) {
				...
				case SIN_ODD:
					...
					break ;
				default:
					cerr << "Unknow theta basis in Domain_compact::export_tau_val_domain_mquant" << endl ;
					abort() ;
				}
			}
		}
...
void Domain_compact::export_tau_val_domain (const Val_domain& so, int mlim, int order, Array<double>& sec, int& pos_sec, int ncond) const {
			...
			for (int j=0 ; j<nbr_coefs(1) ; j++) {
				...
				for (int i=0 ; i<nbr_coefs(0)-order ; i++) {
					...
					case SIN_ODD:
						...
						break ;
					default:
						cerr << "Unknow theta basis in Domain_compact::export_tau_val_domain" << endl ;
						abort() ;
					}
				}
			}
...
void Domain_compact::export_tau (const Tensor& tt, int dom, int order, Array<double>& res, int& pos_res, const Array<int>& ncond,
	...
	switch (val) {
		case 0 :
			if (tt.is_m_quant_affected()) {
				...
				export_tau_val_domain_mquant (tt()(dom), tt.get_parameters().get_m_quant(), order, res, pos_res, ncond(0)) ;
			} 
			else {
		...
		case 1 : {
			...
			if (!found) {
				cerr << "Unknown type of vector Domain_compact::export_tau" << endl ;
				abort() ;
			}
		}
			break ;
		case 2 : {
			...
			if (!found) {
				cerr << "Unknown type of 2-tensor Domain_compact::export_tau" << endl ;
				abort() ;
			}
		}
			break ;
