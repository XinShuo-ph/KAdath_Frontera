			...
			for (int j=0 ; j<nbr_coefs(1) ; j++) {
				...
				switch (baset) {
					...
					case SIN_ODD:
						...
						break ;
					default:
			cerr << "Unknow theta basis in Domain_compact::affecte_tau_val_domain" << endl ;
						abort() ;
					}
...
void Domain_compact::affecte_tau (Tensor& tt, int dom, const Array<double>& cf, int& pos_cf) const {
	...
	switch (val) {
		...
		case 1 : {
			...
			if (!found) {
				cerr << "Unknown type of vector Domain_compact::affecte_tau" << endl ;
				abort() ;
			}
		}
			break ;
		case 2 : {
			...
			if (!found) {
				cerr << "Unknown type of 2-tensor Domain_compact::affecte_tau" << endl ;
				abort() ;
			}
		}
			break ;
