...
int Domain_shell::nbr_unknowns_val_domain_mquant (const Val_domain& so, int mquant) const {
	int res = 0 ;
	...
	do {
		...
		switch (baset) {
					...
					case SIN_ODD:
						...
						break ;
					default:
						cerr << "Unknow theta basis in Domain_shell::nbr_unknowns_val_domain_mquant" << endl ;
						abort() ;
		}
		if (indic)
			res ++ ;
	...
	do {
		...
		switch (baset) {
					...
					case SIN_ODD:
						...
						break ;
					default:
						cerr << "Unknow theta basis in Domain_shell::nbr_unknowns_val_domain" << endl ;
						abort() ;
		}
		if (indic)
			res ++ ;
