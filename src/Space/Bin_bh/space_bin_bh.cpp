...
Space_bin_bh::Space_bin_bh (int ttype, double dist, double rbh1, double rbh2, double rext, int nr) {
    ...
    domains[0] = new Domain_nucleus (0, ttype, rbh1/2., center_minus, res) ; 
    domains[1] = new Domain_shell_outer_homothetic (*this, 1, ttype, rbh1/2., rbh1, center_minus, res) ;    
    ...
    domains[3] = new Domain_nucleus (3, ttype, rbh2/2., center_plus, res) ; 
    domains[4] = new Domain_shell_outer_homothetic (*this, 4, ttype, rbh2/2., rbh2, center_plus, res) ;    
...
Space_bin_bh::Space_bin_bh (FILE* fd) {
	...
	domains[4] = new Domain_shell_outer_homothetic (*this, 4, fd) ;	
	domains[5] = new Domain_shell_inner_homothetic (*this, 5, fd) ;
	...
	// Compactified
	domains[11] = new Domain_compact(11, fd) ;  
...
int Space_bin_bh::nbr_unknowns_from_variable_domains () const {
    ...
    return (domains[2]->nbr_unknowns_from_adapted() + domains[5]->nbr_unknowns_from_adapted()) ;
}
...
Array<int> Space_bin_bh::get_indices_matching_non_std(int dom, int bound) const {
	...
	if (dom== 5) {
		...
		case OUTER_BC :
		  ...
		  break ;
		default :
		  cerr << "Bad bound in Space_bin_bh::get_indices_matching_non_std" << endl ;			abort() ;
		}
	return res ;
	}
