...
Space_fourD_periodic::Space_fourD_periodic(FILE* fd) {
	fread_be (&nbr_domains, sizeof(int), 1, fd) ;
	fread_be (&ndim, sizeof(int), 1, fd) ;
	fread_be (&type_base, sizeof(int), 1, fd) ;
	domains = new Domain* [nbr_domains] ;
	//nucleus :
	domains[0] = new Domain_fourD_periodic_nucleus(0, fd) ;
	//Shells :
	for (int i=1 ; i<nbr_domains ; i++)
		domains[i] = new Domain_fourD_periodic_shell(i, fd) ;
}

Space_fourD_periodic::~Space_fourD_periodic() {
}

void Space_fourD_periodic::save (FILE* fd) const  {
...
double Space_fourD_periodic::get_omega() const {
	  const Domain_fourD_periodic_nucleus* periodicnuc = dynamic_cast<const Domain_fourD_periodic_nucleus*>(domains[0]) ;
