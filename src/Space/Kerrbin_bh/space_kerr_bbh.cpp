...
Space_Kerr_bbh::Space_Kerr_bbh (FILE* fd) {
  ...
  assert(n_shells_outer >= 0);
	...
	domains[BH1+1] = new Domain_shell_outer_adapted (*this, BH1+1, fd) ;
	domains[BH1+2] = new Domain_shell_inner_adapted (*this, BH1+2, fd) ;
  ...
  for(int i = 0; i < n_shells1; ++i)
	...
	domains[BH2+1] = new Domain_shell_outer_adapted (*this, BH2+1, fd) ;
	domains[BH2+2] = new Domain_shell_inner_adapted (*this, BH2+2, fd) ;
...
void Space_Kerr_bbh::save (FILE* fd) const  {
...
Array<int> Space_Kerr_bbh::get_indices_matching_non_std(int dom, int bound) const {
	if (dom==BH1 + n_shells1 + 2) {
	  ...
	  switch (bound) {
		case OUTER_BC :
		  ...
		  break ;
		default :
		  cerr << "Bad bound in Space_Kerr_bbh::get_indices_matching_non_std" << endl ;
	...
	if (dom== BH2 + n_shells2 + 2) {
		...
		case OUTER_BC :
		  ...
		  break ;
		default :
		  cerr << "Bad bound in Space_Kerr_bbh::get_indices_matching_non_std" << endl ;			std::_Exit(EXIT_FAILURE) ;
		}
	return res ;
	}
	...
	cerr << "Bad domain in Space_Kerr_bbh::get_indices_matching_non_std" << endl ;
	std::_Exit(EXIT_FAILURE) ;
}
