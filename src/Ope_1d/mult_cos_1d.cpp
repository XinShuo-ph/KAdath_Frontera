...
int mult_cos_1d_pasprevu (Array<double>&) {
	cout << "mult_cos_1d not implemented." << endl ;
	abort() ;
} 

int mult_cos_1d_cossin (Array<double>& tab) {
	...
	res.set(0) = 0.5*tab(2) ;
	res.set(1) = 0 ;
	res.set(2) = tab(0) + 0.5*tab(4) ;
	res.set(3) = 0.5*tab(5) ;
	for (int k=4; k<nr-2; k++)
	    res.set(k) = 0.5*(tab(2+k) + tab(k-2)) ;
	 
	res.set(nr-2) = 0.5*tab(nr-4) ;
	res.set(nr-1) = 0. ;
...
int mult_cos_1d_cos (Array<double>& tab) {
	assert (tab.get_ndim()==1) ;
	int nr = tab.get_size(0) ;
...
int mult_cos_1d_sin (Array<double>& tab) {
	assert (tab.get_ndim()==1) ;
	int nr = tab.get_size(0) ;
...
int mult_cos_1d_cos_even (Array<double>& tab) {
	assert (tab.get_ndim()==1) ;
	int nr = tab.get_size(0) ;
...
int mult_cos_1d_cos_odd (Array<double>& tab) {
	assert (tab.get_ndim()==1) ;
	int nr = tab.get_size(0) ;
...
int mult_cos_1d_sin_even (Array<double>& tab) {
	assert (tab.get_ndim()==1) ;
	int nr = tab.get_size(0) ;
...
int mult_cos_1d_sin_odd (Array<double>& tab) {
	assert (tab.get_ndim()==1) ;
	int nr = tab.get_size(0) ;
...
int mult_cos_1d (int base, Array<double>& tab) {
    static int (*mult_cos_1d[NBR_MAX_BASE])(Array<double>&) ;
