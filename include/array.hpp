...
#include "index.hpp"
#include "dim_array_iterator.hpp"
...
template <typename T> class Array : public Memory_mapped {
	...
	public:
	    ...
	    Memory_mapped_array<int> steps;///< Steps for index incremention with respect to each dimension.
	...
	public:
	    ...
	    */
	    explicit Array (const Dim_array& res) : dimensions{res}, steps{res.get_ndim()+1}, nbr{1}, data{nullptr} {
	        steps.front() = 1;
            for (int i=0 ; i<res.get_ndim() ; i++) {
                nbr *= res(i) ;
                steps[i+1] = nbr;
            }
            data.resize(nbr) ;
	    ...
	    * The elements are not initialized.
	    */
	    explicit Array (int i)  : dimensions{1}, steps{2}, nbr{i}, data{nbr} {
            steps[0] = 1;
            steps[1] = i;
            dimensions.set(0) = i ;
         }
	    ...
	    * The elements are not initialized.
	    */
        explicit Array (int i, int j)  : dimensions{2}, steps{3}, nbr{steps.back()}, data{nbr} {
            steps[0] = 1;
            steps[1] = i;
            steps[2] = j;
            dimensions.set(0) = i ;
	     ...
	     * The elements are not initialized.
	     */
        explicit Array (int i, int j, int k)  : dimensions{3}, steps{4}, nbr{steps.back()}, data{nbr} {
            steps[0] = 1;
            steps[1] = i;
            steps[2] = j;
            steps[3] = k;
	...
	public:
	    ...
	    //! (Re)-computes the incremental steps.
	    void compute_steps() {steps.front() = 1; for(int i{0};i<nbr;i++) steps[i+1] = steps[i] * dimensions[i];}
...
//            return data[index] ;
//        }
        reference set(const Dim_array_iterator &pos) {return data[pos.position];}
	    /**
...
//            return data[index] ;
//	    }
	    T operator()(Dim_array_iterator const &pos) const {return data[pos.position];}
	    /**
...
//            return data[i*dimensions(1)*dimensions(2)+j*dimensions(2)+k] ;
	   ...
	   */
	    ...
	    */
	    ...
	    /**
	     * @return a reference to constant incremental steps.
	     */
	    auto const & get_steps() const {return steps;}
	...
	T & set(Array_iterator const & i);
	T operator()(Array_iterator const &) const;
} ;
...
// Display
  ...
  /*  template<typename T,bool is_const = std::is_const<T>::value> struct Iterator_traits;
    template<typename T> struct Iterator_traits<T,false> {
        using value_type = T;
        using reference = T &;
        using pointer = T *;
        using container = Array<T>;
        using reference_to_container = Array<T> &;
        static constexpr bool is_const_iterator {false};
        static pointer begin(reference_to_container array) {return array.set_data().begin();}
        static pointer end(reference_to_container array) {return array.set_data().end();}
    };
    template<typename T> struct Iterator_traits<T const,true> {
        using value_type = T;
        using reference = T const &;
        using pointer = T const *;
        using container = Array<T>;
        using reference_to_container = Array<T> const &;
        static constexpr bool is_const_iterator {true};
        static pointer begin(reference_to_container array) {return array.get_data().cbegin();}
        static pointer end(reference_to_container array) {return array.get_data().cend();}
    };
*/
 /*   template<typename T> */class Array_iterator : public Memory_mapped {
    public:
        ...
        using container = typename Iterator_traits<T>::container;
        using reference_to_container = typename Iterator_traits<T>::reference_to_container;
        static constexpr bool is_const{Iterator_traits<T>::is_const_iterator};*/
    protected:
        Memory_mapped_array<int> const & steps;
//        pointer const position;
        int position;
    public:
        template<typename T> Array_iterator(Array<T> const & array, int init = 0)
            : steps{array.get_steps()}, /*position{Iterator_traits<T>::begin(array) + init},*/ position{init} {}
        Array_iterator(Array_iterator const & source) : steps{source.steps}, /*position{source.position},*/ position{source.position} {}
        /**
         * Sets the index of \c this to the value of the \c source. If compiled with checks enabled, the operation
         * will require the steps array to be components-wise equals.
         * @param source \c Array_iterator object owning the \c index value to copy.
         * @return a reference to \c this.
         */
        Array_iterator& operator=(Array_iterator const & source) ;
/*        //! Dereferencement operator.
        reference operator*() const {return position[index];}
        //! Indirection operator.
        pointer operator->() const {return std::addressof(position[index]);}*/
        //! Postfix unit increment operator.
        Array_iterator & operator++() {position++; return *this;}
        //! Postfix unit decrement operator.
        Array_iterator & operator--() {position--; return *this;}
        //! Equality check toward another \c Array_iterator object.
        bool operator==(Array_iterator const & other) const {return position == other.position;}
        //! Inequality check toward another \c Array_iterator object.
        bool operator!=(Array_iterator const & other) const {return position != other.position;}

        int get_position() const {return position;}
        void set_position(int _position) {position = _position;}
        void set(Array_iterator const & i) {assert(&steps == &i.steps); position = i.position;}
        ...
        bool check_value() {
            if(position >= steps.back()) { position = steps.back() - 1; return false;}
            else return true;
        }
    };

   inline Array_iterator & Array_iterator::operator=(Array_iterator const & source) {
#ifdef ALL_CHECKS_ENABLED
#ifdef ENABLE_ASSERTS
        bool assignable {true};
        for(int i{0};i<steps.get_size() && assignable;i++) assignable = steps[i] == source.steps[i];
        assert(assignable);
#endif
#endif
    ...
    template<typename T> inline T & Array<T>::set(Array_iterator const &i) {return data[i.position];}
    template<typename T> inline T Array<T>::operator()(Array_iterator const &i) const {return data[i.position];}
}
#endif
