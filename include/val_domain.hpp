...
namespace Kadath {
    ...
    **/
    ...
    class Val_domain : public Memory_mapped {
    protected:
        const Domain* zone ; ///< Pointer to the associated \c Domain
        Base_spectral base ; ///< Spectral basis of the field.

        mutable bool is_zero ; ///< Indicator used for null fields (for speed issues).
        ...
        mutable Memory_mapped_array<Val_domain*> p_der_var ; ///< Pointers on the derivatives of the field with respect to the numerical coordinates.
        mutable Memory_mapped_array<Val_domain*> p_der_abs ; ///< Pointers on the derivatives of the field with respect to the absolute Cartesian coordinates.
    ...
    public:
        ...
        */
        void set_in_conf() const;
        /**
        * Destroys the values in the configuration space.
        */
        void set_in_coef() const;
        /**
        * Allocates the values in the configuration space and destroys the values in the coefficients space.
        */
        void allocate_conf() const;
        /**
        * Allocates the values in the coefficient space and destroys the values in the configuration space.
        */
        void allocate_coef() const;
    ...
    inline Val_domain::Val_domain(const Domain* dom) :
        zone{dom}, base{zone->get_ndim()}, is_zero{false}, c{zone->get_ndim()}, cf{zone->get_ndim()},
        in_conf{false}, in_coef{false} , p_der_var{zone->get_ndim(),initialize},
        p_der_abs{zone->get_ndim(),initialize}
    {}

    inline Val_domain::Val_domain(Val_domain && so) :
        zone{so.zone}, base{std::move(so.base)}, is_zero{so.is_zero}, c{std::move(so.c)},
        cf{std::move(so.cf)}, in_conf{so.in_conf},  in_coef{so.in_coef},
        p_der_var{std::move(so.p_der_var)}, p_der_abs{std::move(so.p_der_abs)}
    {}
    ...
    inline void Val_domain::annule_hard() {
        allocate_conf() ;
        del_deriv() ;
        c = 0. ;
    }

    inline double& Val_domain::set(const Index& index)  {
        set_in_conf() ;
        return c.set(index) ;
    }

    inline double& Val_domain::set_coef(const Index& index)  {
        set_in_coef() ;
        return cf.set(index) ;
    }

    inline double Val_domain::get_coef(const Index& index) const {
        if (is_zero)
            return 0. ;
        else {
            assert (in_coef) ;
    ...
    inline double Val_domain::operator()(const Index& index) const {
        coef_i() ;
        return c(index) ;
    }

    inline void Val_domain::set_in_conf() const {
//        cf.clear();
        in_conf = true ;
        in_coef = false ;
    }

    inline void Val_domain::set_in_coef() const {
//        c.clear();
        in_conf = false ;
        in_coef = true ;
    }

    inline void Val_domain::allocate_conf() const {
        set_in_conf() ;
        is_zero = false ;
        c.resize(zone->get_nbr_points()) ;
    }

    inline void Val_domain::allocate_coef() const {
        set_in_coef() ;
        is_zero = false ;
        cf.resize(zone->get_nbr_coefs()) ;
    }

    inline Val_domain Val_domain::der_r() const {
        return zone->der_r (*this) ;
    }

    inline Val_domain Val_domain::der_t() const {
        return zone->der_t (*this) ;
    }

    inline Val_domain Val_domain::der_p() const {
        return zone->der_p (*this) ;
    }


    inline Val_domain Val_domain::der_r_rtwo() const {
        return zone->der_r_rtwo (*this) ;
    }

    inline Val_domain Val_domain::div_r() const {
        if (is_zero)
            return *this ;
        else
    ...
    inline Val_domain Val_domain::mult_r() const {
        if (is_zero)
            return *this ;
        else
            return zone->mult_r (*this) ;
    }
    inline double Val_domain::integrale() const {
        if (is_zero)
            return 0. ;
        else
    ...
    inline double Val_domain::integ_volume() const {
        if (is_zero)
            return 0. ;
        else
