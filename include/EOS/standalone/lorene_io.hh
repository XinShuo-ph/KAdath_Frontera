...
static const std::array<double, Hot_Slice::v_index::NUM_VARS + 1> conv{
    {MC::RHOGF * MC::mnuc_cgs * MC::cm3_to_fm3, MC::RHOGF, MC::PRESSGF, 1., 1.,
     1., 1.}};
*/
static std::array<std::vector<double>, 3> Lorene_Table(const std::string &filename, const double h_cut) {
  using namespace Margherita_constants;

  std::ifstream file(filename);
  if (!file.is_open()) {
    std::string msg = "failed to open " + filename;
  ...
  while (std::getline(file, line)) {
    ...
    double rho = n * RHOGF * mnuc_cgs * cm3_to_fm3;
    double eps = e / n / mnuc_cgs / cm3_to_fm3 - 1.;
    double press = p * PRESSGF;
