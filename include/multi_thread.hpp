...
namespace Kadath
{
    ...
    public:
        ...
        void add_eq_matching_exception (int dom, int bb, const char* eq, const Param& par, const char* eq_exception,
                int n_cmp = -1, Array<int>** p_cmp=0x0)
    ...
    template<typename... Args>
    System_of_eqs_threaded::System_of_eqs_threaded(std::size_t max_num_threads,Args && ... args) :
        System_of_eqs{std::forward<Args>(args)...}, thread_specific_systems(max_num_threads-1)
    {
    ...
    void System_of_eqs_threaded::call_base_method(void (System_of_eqs::*method)(Args...), Args... args)
    {
        ...
        for(auto & p_syst : thread_specific_systems)
        {
            ((*p_syst).*method)(std::forward<Args>(args)...);
        }
    }
}
